...
├── js/
│   ├── data.js         — データユーティリティ（定義マスタ/整合性バリデーション）134行
│   ├── graph.js        — Cytoscape.jsグラフ管理（スタイル/レイアウト/操作）634行
│   └── app.js          — アプリケーションコントローラ（データ読込/検索/詳細表示/逆引き）761行
├── data/
│   ├── muscles.json    — 筋データ 222件（9,634行）
│   ├── bones.json      — 骨・軟部組織データ 145件（1,837行）
│   ├── nerves.json     — 神経データ 77件（1,092行）
...
- **エッジ合計: 1,114本**（起始352 + 停止291 + 神経支配248 + 関節結合108 + 靱帯付着87 + 感覚支配28）
...
- **jointTypes**: 関節種類定義 10種（hinge/ball_and_socket/pivot/condyloid/saddle/gliding/suture/gomphosis/symphysis/syndesmosis）
- **validate()**: 全JSONの参照整合性チェック（筋→骨/神経/関節、関節→骨、皮膚→神経、靱帯→骨/関節、神経parent→神経）。アプリ起動時にコンソール出力
...
- **構造監査 4ラウンド（22カテゴリ）**で計144件修正
  - Round 1（A-E）: 解剖学的正確性 — 61件（起始停止45件 + 神経レベル・骨新規追加16件）
  - Round 2（F-L）: スキーマ・コード品質 — 63件（神経レベル整合24件 + 靱帯文字統一36件 + コード修正3件）
  - Round 3（M-Q）: 臨床・テキスト品質 — 20件（主動筋役割昇格15件 + skin英名&修正4件 + 骨trivia1件）
  - Round 4（R-V）: トポロジー・国際標準 — 0件（収束確認）
  - 検証カテゴリ: 解剖学的正確性/スキーマ整合性/region整合性/神経支配正確性/actions整合性/重複欠落/コード品質/UI表示網羅性/テキスト品質/臨床完全性/グラフ可視化/エッジケース耐性/筋グループ完全性/神経階層ツリー/description矛盾/グラフトポロジー/国際標準準拠
