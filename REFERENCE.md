...
│   ├── graph.js        — Cytoscape.jsグラフ管理（スタイル/レイアウト/操作）620行
│   └── app.js          — アプリケーションコントローラ（データ読込/検索/詳細表示/逆引き）738行
├── data/
│   ├── muscles.json    — 筋データ 222件（9,537行）
│   ├── bones.json      — 骨データ 138件（1,747行）
│   ├── nerves.json     — 神経データ 77件（1,087行）
│   ├── joints.json     — 関節データ 36件（672行）
│   ├── ligaments.json  — 靱帯データ 42件（422行）
│   └── skin.json       — 皮膚データ 27件（272行）
└── REFERENCE.md        — 本ファイル
```
...
## データ規模
- **ノード合計: 542件**（筋222 + 骨138 + 神経77 + 関節36 + 靱帯42 + 皮膚27）
- **エッジ合計: 1,085本**（起始337 + 停止286 + 神経支配244 + 関節結合105 + 靱帯付着85 + 感覚支配28）

### 部位別内訳
| 部位 | 筋 | 骨 | 神経 | 関節 | 靱帯 | 皮膚 | 計 |
|------|----|----|------|------|------|------|----|
| 頭頸部 | 71 | 31 | 27 | 4 | 4 | 4 | 141 |
| 上肢 | 49 | 35 | 21 | 15 | 11 | 7 | 138 |
| 下肢 | 55 | 33 | 21 | 10 | 17 | 11 | 147 |
| 体幹 | 18 | 33 | 5 | 4 | 1 | 3 | 64 |
| 背部 | 21 | 1 | 1 | — | 5 | 1 | 29 |
| 骨盤 | 8 | 5 | 2 | 3 | 4 | 1 | 23 |
...
- **buildSearchIndex()**: 全542件のインクリメンタルサーチ用インデックス（日英ラテン名）
- **showDetail(nodeData)**: ノード種別に応じた詳細パネル描画
  - 筋: 起始/停止/作用/神経支配 + 豆知識
  - 骨: ランドマーク/関節/靱帯/起始する筋/停止する筋
...
- **フィルタ**: 左サイドバー（ノードタイプ6種 + エッジタイプ6種 + 部位6種）
